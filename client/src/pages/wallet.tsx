...
export default function WalletPage() {
  ...
  const handleWithdraw = (e: React.FormEvent) => {
    e.preventDefault();
    withdraw.mutate(parseFloat(withdrawAmount), {
      onSuccess: () => {
          ...
          <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
            ...
            <div className="flex gap-3">
              ...
              <Dialog open={withdrawOpen} onOpenChange={setWithdrawOpen}>
                <DialogTrigger asChild>
                  <Button size="lg" variant="outline" className="bg-indigo-500/20 text-white border-white/20 hover:bg-indigo-500/30 font-semibold shadow-lg">
                ...
                <DialogContent className="bg-card border-border/50">
                  ...
                  <form onSubmit={handleWithdraw} className="space-y-4 pt-4">
                    <div className="space-y-2">
                      ...
                      <p className="text-xs text-muted-foreground">Available: ${Number(wallet?.balance || 0).toFixed(2)}</p>
                    </div>
                    <Button type="submit" className="w-full" disabled={withdraw.isPending}>
                      {withdraw.isPending ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : null}
                      Confirm Withdrawal
                    </Button>
                  </form>
                </DialogContent>
